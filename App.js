export class App {
	...
	// view helper function
	completed() {
		const completed = this.state.todosList.reduce(
			(total, todo, index) => (total += todo.completed ? 0 : 1),
			0
		)

		return completed === 1 ? `1 item left` : `${completed} items left`
	}

	// view helper function
	filterBtnClassNames(filterState) {
		let classNames = `filter-${filterState} filter-btn `
		classNames += filterState === this.state.filter ? 'filter-btn--active' : ''
	...
	addEventListeners() {
		...
		const todoCheckboxes = this.container.querySelectorAll(
			'.todo__completed-checkbox'
		)
